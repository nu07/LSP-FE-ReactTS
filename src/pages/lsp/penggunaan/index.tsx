...
export default function Pelanggan() {
  ...
  const getDataPelanggan = async () => {
    try {
      const data = await Axios.get("/penggunaans");
            ...
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody>
                {dataPelanggan.map((person: any, personIdx: any) => (
                  <tr
                    key={person.id_penggunaan}
                    className={personIdx % 2 === 0 ? "bg-white" : "bg-gray-50"}
                  >
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                      {person.id_penggunaan}
                    </td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <Link
                        to={"/penggunaan/" + person.id_pelanggan}
                      ...
                      <button
                        ...
                        onClick={() => (
                          setidDataDelete(person.id_penggunaan),
                          setModalDelete(true)
                        )}
                      >
                        Delete
